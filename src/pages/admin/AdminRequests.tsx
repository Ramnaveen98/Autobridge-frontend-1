...
export default function AdminRequests() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        setErr(null);
        const [reqs, rawAgents] = await Promise.all([
          listAdminRequests(),
          listAgents(),
        ]);
        setRows(Array.isArray(reqs) ? reqs : []);
  ...
  const canAssign = (s: RequestStatus) => s === "PENDING";
  const canReassign = (s: RequestStatus) => s === "ASSIGNED";
  // **Update**: cancel allowed only before COMPLETED; also hide if already CANCELLED
  const canCancel = (s: RequestStatus) => s !== "COMPLETED" && s !== "CANCELLED";

  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      <h1 className="text-xl font-semibold mb-2">Admin Requests</h1>
