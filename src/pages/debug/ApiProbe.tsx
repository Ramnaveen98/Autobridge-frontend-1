...
export default function ApiProbe() {
    ...
    (async () => {
      ...
      for (const url of CAR_CANDIDATES) {
        try {
          ...
          out.push({ url, ok: true, status: 200, size: Array.isArray(arr) ? arr.length : 0 });
          break; // first OK is enough
        } catch (e: any) {
          out.push({ url, ok: false, status: e?.response?.status, note: e?.response?.data ? JSON.stringify(e.response.data) : "" });
        }
      }
  ...
  return (
    <div className="max-w-3xl mx-auto py-8">
      ...
      <p className="text-sm mb-4 text-blue-200/80">Checks multiple candidate endpoints and shows which one responds.</p>
      <div className="space-y-2">
        {rows.map((r, i) => (
          <div key={i} className={`p-3 rounded ${r.ok ? "bg-blue-800/40" : "bg-blue-900/40"}`}>
            <div className="text-sm">
              ...
              {r.status !== undefined && <span className="ml-2 text-blue-200/70">status: {r.status}</span>}
              {r.size !== undefined && <span className="ml-2 text-blue-200/70">items: {r.size}</span>}
            </div>
            {!!r.note && <div className="text-xs opacity-70 break-all mt-1">{r.note}</div>}
          </div>
        ))}
      </div>
