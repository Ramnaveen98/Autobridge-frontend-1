...
export default function Latest() {
  ...
  async function load() {
    ...
    for (const url of PUBLIC_ENDPOINTS) {
      try {
        ...
        setRows(list.slice(0, 3)); // show exactly the first 3
        return;
      } catch (e) { lastErr = e; }
    }
    const st  = lastErr?.response?.status ?? "?";
    const msg = lastErr?.response?.data?.message ?? lastErr?.message ?? "Failed to load latest vehicles";
    setError(`(${st}) ${msg}`);
    // eslint-disable-next-line no-console
    console.error("Latest vehicles load failed:", lastErr);
  }
  ...
  if (error) return <div className="px-6 text-red-400">{error}</div>;
  if (!rows.length) return <div className="px-6 opacity-80">No cars available right now.</div>;
